...
public class TestBorrowedCommand {
    ...
    public void testNoInput() {
        ...
        Expectation: returns ResponseFlag.SUCCESS, but no borrowed books will be queried and printed out
         */
    ...
    public void testWrongIDType() {
        ...
        Expectation:  FAILURE response flag, but no borrowed books will be printed out
         */
        Assertions.assertSame(ResponseFlag.FAILURE, cmd_spy.onExecute(this.execMock, "@%"));
        verify(cmd_spy, never()).getVisitor(any());
        verify(cmd_spy, never()).getBorrowedBooks(any());
    }
    ...
    public void InvalidVisitorID() {
        ...
        Expectation: SUCCESS response flag, but no borrowed books will be queried and printed out
         */
    ...
    public void VisitorHasNoBooks() {
        /*
        Test Explanation: test the command with everything accurate. The visitor has no borrowed books under her account
        Expectation: returns ResponseFlag.SUCCESS, no books being printed out.
         */
        assertSame(ResponseFlag.SUCCESS, cmd_spy.onExecute(execMock, VALID_VISITOR_ID));
        verify(cmd_spy, times(1)).getVisitor(any());
        verify(cmd_spy, Mockito.times(1)).getBorrowedBooks(any());
        verify(cmd_spy, never()).getBookResponse(any(), any());
    }
    @Test
    public void VisitorHasBorrowedBooks() {
        /*
        Test Explanation: test the command with everything accurate. The visitor has borrowed books under her account
        Expectation: returns ResponseFlag.SUCCESS, all borrowed books being printed out.
         */
        this.borrowedList.add(checkoutMock);
        assertSame(ResponseFlag.SUCCESS, cmd_spy.onExecute(execMock, VALID_VISITOR_ID));
        verify(cmd_spy, times(1)).getVisitor(any());
        verify(cmd_spy, Mockito.times(1)).getBorrowedBooks(any());
        verify(cmd_spy, Mockito.times(1)).getBookResponse(any(), any());
    }
